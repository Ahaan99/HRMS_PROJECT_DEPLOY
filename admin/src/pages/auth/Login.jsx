...
@keyframes ringDraw { from{stroke-dashoffset:264} to{stroke-dashoffset:10.6} }
...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
const CONTENT = {
  badge: "Enterprise HRMS Platform",
  headlineTop: "One HRMS. Every",
  headlineGradient: "people process.",
  subtext:
    "Recruitment, onboarding, attendance, payroll and performance — managed end-to-end in one secure, intelligent HRMS built for the whole organisation.",
  marquee: [
    "Talent acquisition",
    "Smart onboarding",
...
const ROLE_META = {
  admin: { label: "Admin", subtitle: "Sign in to the HRMS Admin Panel.", endpoint: "/super-admin/auth/login" },
  manager: { label: "Manager", subtitle: "Sign in to the HRMS Manager Panel.", endpoint: "/super-admin/auth/login-manager" },
  tl: { label: "TL", subtitle: "Sign in to the HRMS Team Lead Panel.", endpoint: "/super-admin/auth/login-tl" },
};
...
/* ═══════════════ Hero Scene — living HR workspace illustration ═══════════════ */
const SCENE = {
  avatars: [
    { i: "AK", c: "from-violet-500 to-fuchsia-500" },
    { i: "PR", c: "from-fuchsia-500 to-pink-500" },
    { i: "SM", c: "from-indigo-500 to-violet-500" },
    { i: "NJ", c: "from-purple-500 to-indigo-500" },
  ],
  bars: [40, 65, 50, 80, 70, 95, 60],
  tasks: ["Offer letter sent", "Onboarding kit ready", "Payroll approved"],
};
...
function HeroScene() {
  return (
    ...
    >
      {/* dotted orbits */}
      <div
        className="lx-anim absolute inset-[4%] rounded-full border border-dashed border-fuchsia-300/25"
      ...
      <div
        className="lx-anim absolute inset-[16%] rounded-full border border-dashed border-violet-300/20"
      ...
      {/* core glow */}
      <div
        className="absolute inset-[22%] rounded-full blur-[70px]"
        style={{ background: "radial-gradient(circle, rgba(217,70,239,.38), transparent 70%)" }}
      />

      {/* dashboard on a stack of slabs */}
      <div
        className="lx-anim absolute left-1/2 top-1/2 flex w-[64%] -translate-x-1/2 -translate-y-[52%] flex-col items-center"
      ...
      >
        ...
        >
          <div className="flex items-center justify-between">
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/50">Team overview</p>
          ...
          <div className="mt-2.5 flex items-center">
            {SCENE.avatars.map((a, i) => (
              <span
                key={a.i}
                className={`lx-anim -ml-1.5 flex h-7 w-7 items-center justify-center rounded-full border-2 border-[#120c22] bg-gradient-to-br ${a.c} text-[9px] font-bold text-white first:ml-0`}
                style={{ animation: `popIn .5s cubic-bezier(.34,1.56,.64,1) ${0.9 + i * 0.1}s both` }}
              >
                {a.i}
              </span>
            ))}
            <span className="ml-2 text-[10px] font-semibold text-white/50">+124 active</span>
          </div>
          ...
          <div className="mt-3 flex items-center gap-3">
            <div className="relative h-14 w-14 shrink-0">
              ...
              <span className="absolute inset-0 flex flex-col items-center justify-center leading-none">
                ...
                <span className="mt-0.5 text-[7px] uppercase tracking-wider text-white/45">present</span>
              </span>
            </div>
            <div className="flex-1">
              <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-white/40">Productivity</p>
        ...
        {[
          ...
          <div
            key={i}
            className={`lx-anim ${s.w} -mt-1 h-5 rounded-xl bg-gradient-to-r ${s.c} shadow-[0_18px_30px_-12px_rgba(0,0,0,.7)]`}
            style={{
      ...
      >
        ...
        <div className="leading-tight">
          <p className="text-[11px] font-semibold text-white">Leave approved</p>
          <p className="text-[9px] text-white/45">Riya · 2 days · Sept 14</p>
        </div>
      </div>

      {/* floating: onboarding checklist */}
      <div
        className="lx-anim absolute right-0 top-[18%] hidden w-[42%] rounded-2xl border border-white/12 bg-[#120c22]/90 p-3 backdrop-blur-xl sm:block"
      ...
      >
        <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-white/45">Onboarding</p>
      ...
      {/* floating: payroll progress */}
      <div
        className="lx-anim absolute bottom-[10%] left-[2%] w-[44%] rounded-2xl border border-white/12 bg-[#120c22]/90 p-3 backdrop-blur-xl"
      ...
      >
        <div className="flex items-center justify-between">
          <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-white/45">Payroll · Sept</p>
          <span className="text-[9px] font-bold text-emerald-300">Processed</span>
        </div>
        <p className="mt-1 text-base font-black text-white">₹7.24L</p>
      ...
      {/* floating: new hire */}
      <div
        className="lx-anim absolute bottom-[6%] right-[2%] hidden items-center gap-2.5 rounded-2xl border border-white/12 bg-[#120c22]/90 px-3 py-2.5 backdrop-blur-xl sm:flex"
      ...
      >
        <span className="flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br from-fuchsia-500 to-violet-500 text-[9px] font-bold text-white">RK</span>
        <div className="leading-tight">
          <p className="text-[11px] font-semibold text-white">Rahul joined IT</p>
...
export default function Login() {
  ...
  const [otpStep, setOtpStep] = useState(null); // { admin_id }
  ...
  const finishLogin = (res) => {
    ...
    if (r === "MANAGER") navigate("/manager-dashboard");
    else if (r === "TL") navigate("/tl-dashboard");
    else navigate("/");
  };
  ...
  const handleLogin = async (e) => {
    ...
    try {
      setLoading(true);
      const { data: res } = await API.post(role.endpoint, { email, password });
      if (!res?.success) throw new Error(res?.message || "Login failed");
      if (res.requires_otp) {
        setOtpStep({ admin_id: res.admin_id });
        setOtp("");
        toast("Enter the OTP sent to your registered email/phone");
        return;
      }
      finishLogin(res);
    } catch (err) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      setLoading(true);
      const { data: res } = await API.post("/super-admin/auth/verify-otp", { admin_id: otpStep.admin_id, otp });
  ...
  return (
    ...
    >
      ...
      <header className="lx-rise relative z-10 flex shrink-0 items-center justify-between px-5 py-4 sm:px-8 lg:px-12">
        <div className="flex items-center gap-3">
          ...
          <div className="leading-none">
            <span className="block text-lg font-bold tracking-wide">HRMS</span>
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-7xl flex-1 flex-col items-center justify-center gap-10 px-5 py-6 sm:px-8 lg:min-h-0 lg:flex-row lg:gap-12 lg:px-12 lg:py-2 xl:gap-20">
        ...
        <MagicCard className="w-full max-w-md">
          <div className="p-5 sm:p-7">
            <div className="grid grid-cols-3 gap-1 rounded-xl border border-white/10 bg-black/40 p-1" role="tablist" aria-label="Login role">
              {Object.entries(ROLE_META).map(([key, meta]) => (
                <button
                  ...
                  className={`rounded-lg py-2 text-sm font-semibold transition-all duration-300 disabled:cursor-not-allowed sm:py-2.5 ${
                    loginType === key
            ...
            <p className="mt-1 text-sm text-white/45">{role.subtitle}</p>

            <div className="mt-5">
              {otpStep ? (
                <form onSubmit={handleVerifyOtp} className="flex flex-col gap-4">
                  <div>
                    <label htmlFor="login-otp" className={LABEL_CLASS}>One-Time Password</label>
                    <input
                      id="login-otp"
                      type="text"
                      inputMode="numeric"
                      ...
                      className="w-full rounded-xl border border-white/10 bg-black/40 py-3 text-center text-lg font-bold tracking-[0.4em] text-white placeholder-white/20 outline-none transition-all duration-300 focus:border-fuchsia-400/60 focus:bg-black/60 focus:shadow-[0_0_0_3px_rgba(217,70,239,.15),0_8px_30px_-10px_rgba(217,70,239,.4)] sm:py-3.5"
                    />
                    <p className="mt-2 text-xs text-white/35">OTP expires in 5 minutes. It was sent to your registered email/phone only.</p>
                  </div>
                  <button disabled={loading || otp.length !== 6} className={BTN_CLASS}>
                    <Sheen />
                  ...
                  <button type="button" onClick={() => setOtpStep(null)} className="w-full text-center text-sm text-white/40 transition hover:text-white">
                    Back to login
                  </button>
                </form>
              ) : (
                <form onSubmit={handleLogin} className="flex flex-col gap-4">
                  <div>
                    ...
                    <div className="group relative">
                      ...
                      <input id="login-email" type="email" required autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@hrms.com" className={INPUT_CLASS} />
                    </div>
                  </div>
                  ...
                  {/* Local-dev hint only: stripped from production builds so no credential
                      ever ships on the public Super Admin login page. */}
                  {import.meta.env.DEV && (
                    <div className="rounded-xl border border-white/10 bg-black/30 px-4 py-3">
                      <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/40">Local dev credentials</p>
                      <div className="mt-1.5 flex flex-wrap gap-x-5 gap-y-1 text-sm text-white/60">
                        <p>Email: <span className="font-semibold text-white">admin@hrms.com</span></p>
      ...
      <footer className="relative z-10 shrink-0 border-t border-white/[0.06] py-3">
        ...
        <p className="sr-only">Talent acquisition, smart onboarding, payroll intelligence, attendance and leave, performance analytics, enterprise security</p>
      </footer>
    </div>
  );
}
