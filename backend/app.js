...
// Liveness + DB check used by README-DEPLOY step 1d and the fresh-boot test.
app.get("/api/health", async (req, res) => {
  try {
    await db.query("SELECT 1");
    res.json({ status: "healthy", db: "up", uptime: Math.round(process.uptime()), version: process.env.npm_package_version || null });
  } catch (e) {
    res.status(503).json({ status: "degraded", db: "down", error: e.code || e.message });
  }
});
