...
import { resolveShiftForEmployee, checkInStatus } from "../hr/attendance/shift.helper.js";
...
export const geoPunch = async (req, res) => {
  try {
    ...
    if (!existing) {
      const shift = await resolveShiftForEmployee(empId);
      await db.query(
        `INSERT INTO super_admin_attendance
         (employee_id, employee_name, date, check_in, status, method, check_in_lat, check_in_lng, geo_status, office_id, shift_id)
         VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
        [
          empId,
          empName,
