...
export const getItEmployees = async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT e.id, e.name, e.employeeCode, d.name AS department,
       ...
       WHERE e.isActive = 1 ORDER BY e.name`,
    );
    res.json(rows);
  } catch (err) {
...
export const createCodeReview = async (req, res) => {
  try {
    ...
    if (reviewer_id) {
      if (Number(reviewer_id) === Number(req.employee?.id))
        return res.status(400).json({ message: "You cannot review your own pull request" });
      if (!(await isEngineer(reviewer_id)))
        return res.status(400).json({ message: "Reviewer must be a member of the engineering team" });
    }
    const [r] = await db.query(
...
export const updateCodeReview = async (req, res) => {
  try {
    ...
    const [[review]] = await db.query(
      "SELECT author_id, reviewer_id FROM it_code_reviews WHERE id=?",
      [req.params.id],
    );
    if (!review) return res.status(404).json({ message: "Review not found" });

    const me = Number(req.employee?.id);
    if (me === Number(review.author_id))
      return res.status(403).json({ message: "Authors cannot change the status of their own pull request" });
    if (review.reviewer_id && Number(review.reviewer_id) !== me)
      return res.status(403).json({ message: "Only the assigned reviewer can update this review" });
    if (!review.reviewer_id && !(await isEngineer(me)))
      return res.status(403).json({ message: "Only engineering team members can review pull requests" });

    // First engineer to act on an "any reviewer" request claims it
    await db.query(
      "UPDATE it_code_reviews SET status=?, comments=?, reviewer_id=COALESCE(reviewer_id, ?) WHERE id=?",
      [status, comments || null, me, req.params.id],
    );
    // Merged PR => linked task is Done; re-opened review => back to Review
