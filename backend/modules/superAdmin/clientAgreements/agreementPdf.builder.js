...
/**
 * Render the branded Master Service Agreement PDF into uploads/generated.
 * Shared by the Client Agreements page and the Client Onboarding pipeline.
 * Returns { pdfUrl, agreementNumber, fileName }.
 */
export async function buildProfessionalAgreementPdf({
  ...
  if (!client_company_name || !client_representative_name || !effective_date) {
    const e = new Error("Client company, representative and effective date are required");
    e.status = 400;
    throw e;
  }
  ...
  return { pdfUrl, agreementNumber, fileName: generatedFileName };
}
