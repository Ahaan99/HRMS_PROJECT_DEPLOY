...
const assetUrl = (p) => (!p ? null : /^https?:/i.test(p) ? p : `${API_ORIGIN}${p}`);
...
function HeroCard({ data, onAvatarChanged, onEdit, onPassword }) {
  ...
  const upload = async (e) => {
    ...
    setUploading(true);
    try {
      await API.post("/employee/profile/avatar", fd, { headers: { "Content-Type": "multipart/form-data" } });
